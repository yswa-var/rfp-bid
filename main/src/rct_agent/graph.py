...
from rct_agent.context import Context
from rct_agent.state import InputState, State
from rct_agent.tools import TOOLS
from rct_agent.utils import load_chat_model
...
def route_approval(state: State) -> Literal["tools", "call_model"]:
    ...
    """
    # Check if pending_operation is None (approval granted)
    ...
    # Default to tools if we're not sure (safer to try executing)
    return "tools"
